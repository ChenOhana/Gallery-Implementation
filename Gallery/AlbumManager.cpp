...
{
	m_dataAccess.open();
	this->m_nextUserId = m_dataAccess.getNextUserId();
	this->m_nextAlbumId = m_dataAccess.getNextAlbumId();
	this->m_nextPictureId = m_dataAccess.getNextPictureId();
}
...
void AlbumManager::createAlbum()
{
	...
	Album newAlbum(++m_nextAlbumId, userId,name);
...
void AlbumManager::addPictureToAlbum()
{
	...
	Picture picture(++m_nextPictureId, picName, m_openAlbum.getId());
...
void AlbumManager::showPicture()
{
	...
	displayPicture(pic.getPath());
}

void AlbumManager::displayPicture(string imagePath)
{
	...
	std::cout << std::endl << "Choose Program:" << std::endl;
	...
	if (choice == '1')
	{
		cmd = "C:/WINDOWS/system32/mspaint.exe " + imagePath;
	}
	else if (choice == '2')
	{
		cmd = "C:/Program Files (x86)/IrfanView/i_view32.exe " + imagePath;
	}

	if (CreateProcess(NULL, (LPSTR)cmd.c_str(), NULL, NULL, TRUE, 0, NULL, NULL, &info, &processInfo))
	{
		DWORD dwLen = 256, dwWaitStatus;
		BOOL bWait = TRUE;
		HANDLE hNotif;
		hNotif = FindFirstChangeNotification(imagePath.substr(0, imagePath.find_last_of("/\\")).c_str(), TRUE, FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES
			| FILE_NOTIFY_CHANGE_SIZE | FILE_NOTIFY_CHANGE_LAST_WRITE);
		
		if (hNotif != NULL)
		{
			dwWaitStatus = WaitForSingleObject(hNotif, 5000); //I assume that if the user opened the picture for at least 5 seconds, then he is editing the picture.
			...
			case WAIT_OBJECT_0:
				...
				{
					throw MyException("Error: " + std::to_string(GetLastError()));
				}
				else
				{
					std::cout << std::endl << "Changes were made" << std::endl;
					system("pause");
				}
				break;
			default:
	...
	{
		std::cout << "Couldn't open picture" << GetLastError() << std::endl;
	}
}
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Average Tags per Album: " << m_dataAccess.averageTagsPerAlbumOfUser(user) << std::endl << "  + Number of owned albums: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl;
}
